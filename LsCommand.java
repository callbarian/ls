	...
	*/
		...
		//	Path path = Paths.get(System.getProperty("user.dir"));
			...
			if(cmd.hasOption("t")) {
				...
				for(String string : allFilesList) {
					...
					String fileString = iterator.next();
					Path path = Paths.get(fileString);
				
					String format = "%-20s%-35s%s%s%n";
					if(cmd.hasOption("h")) {
						System.out.printf(format,string,(FileTime)Files.getLastModifiedTime(path, NOFOLLOW_LINKS)
								,Files.getAttribute(path, "basic:size", NOFOLLOW_LINKS),HardLink.getLinkCount(new File(fileString)));
	...
	private Options createOptions() {
		...
		*/
		...
		options.addOption("l",false,null);
